...
const JobPosting = () => {
  ...
  const loadSavedData = () => {
    ...
    if (savedData) {
      try {
        ...
        console.log('Loaded saved job data:', parsed);
  ...
  const [jobData, setJobData] = useState(loadSavedData);
  ...
  useEffect(() => {
    console.log('JobPosting component mounted');
    console.log('Initial jobData:', jobData);
    console.log('Session storage jobPostingData:', sessionStorage.getItem('jobPostingData'));
  }, []);
  ...
  const saveToSessionStorage = (data) => {
    try {
      ...
      console.log('Saved job data to session storage:', dataToSave);
    } catch (error) {
  ...
  useEffect(() => {
    // Only save if jobData has actual content and at least one field is filled
    if (jobData && Object.keys(jobData).length > 0) {
      const hasContent = Object.values(jobData).some(value => 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3) Post job now
      const response = await apiService.post('/enquiry', finalJobData);

      if (response.success) {
        showSuccess('Job posted successfully! You will be contacted by verified experts soon. We are now showing you relevant businesses for your job.', 'Job Posted Successfully');
           ...
           <div className="pt-6 md:pt-[35px] flex flex-col sm:flex-row gap-4">
             ...
             >
              ...
              <button
                ...
                className="w-full sm:w-[150px] h-[46px] rounded-[10px] flex items-center justify-center bg-gray-500 hover:bg-gray-600 text-white"
              >
                <span className="text-base md:text-[18px] font-semibold font-roboto">
                  Debug Storage
               </span>
             </button>
            </div>
