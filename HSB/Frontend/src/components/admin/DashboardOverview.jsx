...
const DashboardOverview = () => {
  ...
  useEffect(() => {
    if (businesses.length > 0) {
      ...
      const verifiedBusinesses = businesses.filter(business => business.verified).length;
      const totalReviews = businesses.reduce((sum, business) => sum + business.reviews, 0);
      const averageRating = businesses.reduce((sum, business) => sum + business.rating, 0) / totalBusinesses;
          ...
          <div className="mt-3">
            <div className="flex items-center">
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div 
                  className={`h-2 rounded-full ${color}/60`}
                  style={{ width: `${trend}%` }}
                ></div>
              </div>
  ...
  const RecentActivity = () => {
    ...
    const recentActivities = [
      ...jobActivities,
      { id: 'business1', type: 'business_added', message: 'New business "Elite HVAC" registered', time: '2 hours ago' },
      { id: 'review1', type: 'review_added', message: 'New review for "Toronto HVAC Experts"', time: '4 hours ago' },
      { id: 'business2', type: 'business_verified', message: 'Verified "Climate Control Masters"', time: '6 hours ago' },
  ...
  const TopBusinesses = () => {
    ...
    return (
      <div className="bg-white shadow-xl rounded-2xl border border-gray-100">
        <div className="px-6 py-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-gray-900">Top Rated Businesses</h3>
            <button className="text-sm text-[#AF2638] hover:text-[#c73545] font-medium">
              View All
            </button>
          </div>
          <div className="space-y-4">
            {topBusinesses.map((business, index) => (
              <div key={business.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                <div className="flex items-center">
                ...
                <div className="flex items-center space-x-3">
                  <div className="text-right">
                    <div className="flex items-center">
                      <span className="text-sm font-bold text-gray-900 mr-1">{business.rating}</span>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-200 text-lg"> 
              Monitor your HVAC directory platform performance and manage businesses, users, and analytics.
            </p>
